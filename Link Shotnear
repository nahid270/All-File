...
def admin_panel():
    ...
    <style> 
        body {{ font-family: 'Plus Jakarta Sans', sans-serif; background: #f8fafc; padding-bottom: 80px; }} 
    ...
    <body class="flex flex-col min-h-screen">
        ...
        <div class="hidden lg:flex flex-col w-72 bg-white border-r h-screen fixed left-0 top-0 p-6 z-50">
            ...
            <nav class="space-y-2 flex-1">
                <button onclick="showTab('overview')" id="desk-overview" class="w-full text-left p-4 rounded-xl font-bold text-slate-500 hover:bg-slate-50 active-tab">📊 Analytics</button>
                <button onclick="showTab('config')" id="desk-config" class="w-full text-left p-4 rounded-xl font-bold text-slate-500 hover:bg-slate-50">⚙️ Settings</button>
                <button onclick="showTab('bulk')" id="desk-bulk" class="w-full text-left p-4 rounded-xl font-bold text-slate-500 hover:bg-slate-50">🚀 Bulk Tools</button>
                <button onclick="showTab('links')" id="desk-links" class="w-full text-left p-4 rounded-xl font-bold text-slate-500 hover:bg-slate-50">🔗 Direct Links</button>
            </nav>
        ...
        <div class="flex-1 p-4 md:p-8 lg:ml-72">
            ...
            <div id="overview" class="tab-content active space-y-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="bg-white p-6 rounded-[25px] shadow-sm border border-slate-100">
                        ...
                        <h3 class="text-3xl font-black text-slate-900">{len(all_urls)}</h3>
                    </div>
                    <div class="bg-blue-600 p-6 rounded-[25px] shadow-lg text-white">
                        <p class="text-blue-200 text-[10px] font-bold uppercase">Clicks</p>
                ...
                <form action="/admin/update" method="POST" class="space-y-6">
                    <div class="bg-white p-6 rounded-[30px] shadow-sm space-y-5 border border-slate-100">
                        ...
                        <div class="p-4 bg-purple-50 rounded-2xl border border-purple-100">
                            <label class="text-[10px] font-bold text-purple-600 mb-2 block uppercase">Template Style</label>
                            <select name="template_style" class="w-full bg-white p-3 rounded-xl font-bold text-slate-700 text-sm outline-none">
                                ...
                                <option value="download" {"selected" if settings.get("template_style")=="download" else ""}>Fake Download</option>
                            </select>
                        </div>

                        <div class="grid grid-cols-2 gap-3">
                            <div><label class="text-[10px] font-bold text-slate-400">Home Color</label>
                            ...
                            <div><label class="text-[10px] font-bold text-slate-400">Step Color</label>
            ...
            <div id="bulk" class="tab-content space-y-6">
                <div class="bg-white p-6 rounded-[30px] shadow-sm border border-slate-100">
                    ...
                    <form action="/admin/bulk_shorten" method="POST">
                        <textarea name="bulk_urls" class="w-full h-48 p-4 bg-slate-50 rounded-2xl font-mono text-xs border-none outline-none mb-4" placeholder="Paste links here (one per line)"></textarea>
                        <button class="w-full bg-blue-600 text-white p-4 rounded-2xl font-black uppercase shadow-lg">Shorten All</button>
                    </form>
                </div>
            </div>
            ...
            <div id="links" class="tab-content space-y-6">
                 <div class="bg-white p-6 rounded-[30px] shadow-sm border border-slate-100">
                     ...
                     <form action="/admin/add_direct_link" method="POST" class="flex flex-col gap-3 mb-6">
                        <input type="url" name="direct_link_url" placeholder="Paste Direct Link..." required class="p-4 bg-purple-50 rounded-2xl border-none font-bold text-slate-700 text-sm">
                        <div class="flex gap-2">
                            <select name="country" class="flex-1 p-3 bg-purple-50 rounded-2xl border-none font-bold text-slate-700 text-xs">
                                <option value="Global">🌍 Global</option>
                                <option value="US">🇺🇸 USA</option>
                                <option value="GB">🇬🇧 UK</option>
                            </select>
                            <select name="device" class="flex-1 p-3 bg-purple-50 rounded-2xl border-none font-bold text-slate-700 text-xs">
                                <option value="All">📱 All</option>
                                <option value="Mobile">📱 Mobile</option>
                                <option value="Desktop">💻 PC</option>
                            </select>
                        </div>
                        <button class="bg-purple-600 text-white p-4 rounded-2xl font-black uppercase text-sm shadow-md">Add Link</button>
                     </form>
                     ...
                     <div class="space-y-3">
                        ...
                        <div class="flex items-center justify-between bg-slate-50 p-4 rounded-2xl border border-slate-100">
                            <div class="flex flex-col overflow-hidden mr-3">
                                <span class="text-xs font-mono text-slate-600 truncate w-32">{dl['url']}</span>
                                <div class="flex gap-1 mt-1">
                                    <span class="text-[9px] font-black uppercase bg-blue-100 text-blue-600 px-1.5 rounded">{dl.get('country', 'Glo')}</span>
                                    <span class="text-[9px] font-black uppercase bg-orange-100 text-orange-600 px-1.5 rounded">{dl.get('device', 'All')}</span>
                                </div>
                            </div>
                            <a href="/admin/delete_direct_link/{dl['_id']}" class="bg-red-100 text-red-600 px-3 py-2 rounded-xl text-[10px] font-black hover:bg-red-200">DEL</a>
                        </div>
        ...
        <div class="lg:hidden fixed bottom-0 w-full bg-white border-t flex justify-around p-3 z-50 shadow-[0_-5px_20px_rgba(0,0,0,0.05)] pb-6">
            ...
            <button onclick="showTab('config')" id="mob-config" class="flex flex-col items-center gap-1 text-slate-400">
                <span class="text-xl">⚙️</span>
                <span class="text-[10px] font-bold">Settings</span>
            </button>
            <button onclick="showTab('bulk')" id="mob-bulk" class="flex flex-col items-center gap-1 text-slate-400">
                <span class="text-xl">🚀</span>
                <span class="text-[10px] font-bold">Bulk</span>
            </button>
            <button onclick="showTab('links')" id="mob-links" class="flex flex-col items-center gap-1 text-slate-400">
                <span class="text-xl">🔗</span>
